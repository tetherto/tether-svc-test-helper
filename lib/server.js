...
class Mocks {
  ...
  handle (req, res) {
    ...
    req.on('end', () => {
      ...
      this.debug && console.log('[DEBUG] tether-svc-test-helper:', str)
      ...
      this.getMock(req, res, parsed, (err, matches) => {
        if (err) {
          console.error('tether-svc-test-helper: route matching error')
          throw err
        }

        if (matches.length === 0) {
          throw new Error(`tether-svc-test-helper: no route matched (${req.url})`)
        }

        if (matches.length > 1) {
          throw new Error(`tether-svc-test-helper: multiple routes matched (${req.url})`)
        }
        ...
        mock.test(req, res, parsed, (err) => {
          if (err) {
            console.error('tether-svc-test-helper: client request test failed')
            throw err
          }
