...
class Client {
  constructor (worker, opts = {}) {
    ...
    const topicConf = this.worker.conf.topicConf
    const capability = topicConf.capability
    this.capability = Buffer.from(capability, 'hex')
  ...
  async request (method, data, opts = {}) {
    ...
    if (this.capability) {
      requestOpts.capability = this.capability
    }

    while (attempts < maxRetries) {
      attempts++
      try {
        return await this.net.jRequest(this.rpcPublicKey, method, data, requestOpts)
      } catch (err) {
