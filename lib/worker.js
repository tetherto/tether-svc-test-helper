...
function createWorker (conf) {
  ...
  const ctx = {
    ...
    ...(conf.chain && { chain: conf.chain }),
    ...(conf.rack && { rack: conf.rack }),
    ...(conf.uri && { uri: conf.uri }),
    ...(conf.debug && { debug: conf.debug }),
    ...(conf.cluster && { cluster: conf.cluster }),
    ...('proc-rpc' in conf && { 'proc-rpc': conf['proc-rpc'] })
  }
